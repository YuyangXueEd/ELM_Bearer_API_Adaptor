...
1. Check that the ELM key is available to the process. In a fresh PowerShell terminal, run `[bool]$env:ELM_API_KEY`. This prints only true or false. A true result in the terminal does not prove the IDE extension host inherited the same environment.
2. Create a new Codex conversation. For CLI/ACP, use `/status` to inspect the model and provider. If the VS Code version has no equivalent command, use its settings, output or diagnostic logs. Expect provider `elm`, model `gpt-5.3-codex` and request host `elm.edina.ac.uk`.
...
| ACP cannot start | Node/npm availability, global command PATH, command path, adapter version and logs |
